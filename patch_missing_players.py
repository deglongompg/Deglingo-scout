...
# Rattrapage des joueurs manquants par slug direct
# Lance: python3 patch_missing_players.py
    ...
    for attempt in range(3):
        try:
            ...
            if "error" in data and "too many" in str(data.get("error","")).lower():
                print(f"    ⏳ Rate limited, pause 45s...")
                time.sleep(45)
                continue
            if "errors" in data:
...
def fetch_player(slug):
    ...
    raw = q("""
    query P($slug: String!) {
        football { player(slug: $slug) {
            displayName position age country { code }
            activeClub { name }
    ...
    except: print("❌ query failed"); return None
    if not player: print("❌ null"); return None
    
    scores_raw = player.get("so5Scores", [])
    played = [s for s in scores_raw if s.get("score",0) > 0]
    ...
    apps = stats.get("appearances", len(played))
    ...
    ATT={"ontarget_scoring_att","pen_area_entries","successful_dribble","was_fouled","penalty_won"}
    aa_profile = {}
    try:
    ...
    pos=player["position"];ecart=std(s15)
    ga=(stats.get("goals",0)+stats.get("assists",0))/apps if apps>0 else 0
    ad=aa_profile.get("aa_defending",0);ap=aa_profile.get("aa_passing",0)
    at=aa_profile.get("aa_attacking",0);ao=aa_profile.get("aa_possession",0)
    has="aa_defending" in aa_profile
    ...
    if pos=="Goalkeeper": arch="GK"
        ...
        elif has and at>ad and ga>=0.25: arch="MIL Créateur"
    ...
    kpis = {
        "name":player["displayName"],"slug":slug,"position":pos,"age":player.get("age",0),
        ...
        "ecart_5":round(std(s5),1),"ecart_15":round(ecart,1),
        ...
        "matchs_played":len(played),"matchs_total":len(scores_raw),
    }
...
# Missing players with their Sorare slugs
# If slug is wrong, the script will try alternatives
MISSING = [
    # PSG
    ("willian-pacho", "PSG DEF"),
    ("theo-hernandez", "PSG DEF"),
    # Lyon  
    ("corentin-tolisso", "OL MIL"),
    ("pavel-sulc", "OL MIL/ATT"),
    ("geoffrey-kondogbia", "OM MIL"),
    # OM
    ("quinten-timber", "OM MIL"),
    # Lille
    ("noah-ngoy", "LOSC DEF"),
    ("calvin-verdonk", "LOSC DEF"),
    # Monaco
    ("simon-adingra", "Monaco ATT"),
    ("aleksandr-golovin", "Monaco MIL"),
    # Extras - key players potentially missing
    ("kang-in-lee", "PSG MIL"),
    ("ousmane-dembele", "PSG ATT"),
    ("marquinhos-silva", "PSG DEF"),
    ("lucas-beraldo", "PSG DEF"),
]
